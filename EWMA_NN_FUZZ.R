...
callMonte.function<-function(r, sigma, t, K){
  N=252
  j=N*t
  s=sigma/sqrt(N)
  mn=r/N-s^2/2
  y<-rep(0, j+1)
  y[1]=1
  y2=exp(cumsum(rnorm(j, mean=mn, sd=s)))
  y[1:j+1]<-y2
  x=(0:j)/N
...
graphics.off()
y1=as.xts(get.hist.quote(instrument = "^gspc",start = "2014-12-09",end = "2018-11-28",quote="AdjClose"))
y1<-y1$Adjusted
...
######################################################################################Metrhod I
call.hn<-HNGOption(TypeFlag = "c", model= mle$model,S=1, X=1, Time.inDays=63, r.daily=0.05/252)
...
call1<-rep(0, 10000)
for(i in 1:10000){
  call1[i]<-callMonte.function(0.05,volatility , 63/250, 1)
}
mean(call1)
...
call2<-rep(0, 10000)
for(i in 1:10000){
  call2[i]<-callMonte.function(0.05,sigma2*sqrt(252) , 63/252, 1)
}
...
call3<-rep(0, 10000)
for(i in 1:10000){
  call3[i]<-callMonte.function(0.05,mean.sigma.nnetar*sqrt(252) , 63/252, 1)
}
